...
class Neutron(Plugin):
    ...
    option_list = [
        ("log", "Gathers all Neutron logs", "slow", False),
        ("quantum", "Overrides checks for newer Neutron components", "fast", False)]

    component_name = "neutron"

    def setup(self):
        if not os.path.exists("/etc/neutron/") or \
                self.get_option("quantum", True):
